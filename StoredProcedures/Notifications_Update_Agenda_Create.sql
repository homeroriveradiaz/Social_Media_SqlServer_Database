...
IF OBJECT_ID(N'Notifications_Update_Agenda_Create', N'P') IS NULL BEGIN
	EXEC('CREATE PROC dbo.Notifications_Update_Agenda_Create AS SELECT 1;');
END;
GO
/**************************************************************************
ALLOWS A RESPONDER TO RAISE NOTIFICATION TO POSTING AUTHOR
**************************************************************************/
ALTER PROCEDURE [dbo].[Notifications_Update_Agenda_Create] (
	@UserID BIGINT
)
AS
